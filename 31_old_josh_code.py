...
'''
DEBUG_TO_FILE = True
def write_debug(num, suit, hand=None, drawn_card=None):
    if not DEBUG_TO_FILE:
        return
    with open("old_output.txt", "a") as f:
        hand_str = repr(hand + [drawn_card])
        f.write(f"{repr(hand):<60}{repr(drawn_card):>20}     {num} {suit}\n")
...
with open("old_output.txt", "w") as f:
    f.write("Deck: " + repr(deck) + "\n\n")
            ...
            if upcardsuit != deck[e][1]:
                ...
                if upcardnum >= (deck[e][0] + deck[e+1][0] + deck[e+2][0]):
                    ...
                    if upcardnum == deck[e][0]:
                        upcardsuit = deck[e][1]
                        setup1()
                        write_debug(upcardnum, upcardsuit, deck[e-3:e], deck[-o])
                        continue
